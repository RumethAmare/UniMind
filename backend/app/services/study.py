...
class StudyService:
    ...
    def _prompt(self, artifact_type: ArtifactType, context: str) -> str:
        instructions = {
            ...
            ArtifactType.MCQS: (
                "Create exactly 10 MCQs as "
                "{'mcqs':[{'question':'','options':['','','',''],'correct_answer':'','explanation':''}]}. "
                "Each MCQ must have exactly 4 options. The correct_answer value must exactly match "
                "one option string, including spelling and capitalization."
            ),
